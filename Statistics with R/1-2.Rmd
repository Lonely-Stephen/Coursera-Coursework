...
sfo_feb_flights %>%
  group_by(carrier) %>%
    summarise(mid = median(arr_delay), IQR = IQR(arr_delay), n = n())
```
...
nycflights %>%
  group_by(month) %>%
  summarise(mid = median(dep_delay)) %>%
  arrange(desc(mid))
```
...
nycflights %>%
  summarise(prop = sum(dep_type == "delayed" & arr_type == "on time")/sum(dep_type=="delayed"))
```
